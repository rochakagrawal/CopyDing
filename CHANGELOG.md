# Changelog

## 1.2.0

- Added an optional failure alert for mouse-based Copy actions
- Detects standard Copy menu items and properly labelled Copy buttons
- Monitors left-button presses only when the option is enabled
- Does not monitor mouse movement or modify mouse events
- Added success-sound modes for Off, ⌘C only, and Any clipboard change

## 1.1.0

- Added Fast, Normal, Relaxed, and Slow Apps timing presets
- Remembered the selected timing between launches
- Added a proper application icon
- Added universal Apple Silicon and Intel release builds

## 1.0.0

- Initial CopyDing release
- Alert when Command-C does not update the clipboard
- Menu-bar controls, system alert test, and Launch at Login
